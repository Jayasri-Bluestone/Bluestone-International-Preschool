  ...
  return (
    <div className="bg-[#FFFFFF] overflow-hidden">
      ...
      <section id="why" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative">
            <img src={franImg} alt="Why Choose Franchise" className="rounded-3xl shadow-2xl w-full object-cover" />
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            ...
            <button onClick={scrollToEnquiry} className="mt-8 px-8 py-3 rounded-full bg-orange-400 text-white font-bold shadow-lg hover:bg-orange-600 transition">
              Become a Franchise Partner
            </button>
          </motion.div>
        </div>
      ...
      <section id="franchise-enquiry" className="py-28 bg-gradient-to-r from-purple-800 to-orange-500">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-30 items-start">
            {/* ENQUIRY FORM */}
            <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-white rounded-[2.5rem] shadow-2xl p-10">
              <h3 className="text-3xl font-extrabold text-center text-purple-800 mb-2">Franchise Enquiry</h3>
              <p className="text-gray-500 mb-8 text-center">Fill the form and our team will get back to you within 24 hours.</p>
              
              <form className="space-y-4" onSubmit={handleSubmit}>
                <input name="fullName" value={formValues.fullName} onChange={handleChange} placeholder="Full Name" required className="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:ring-2 focus:ring-purple-500 outline-none transition" />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input name="email" type="email" value={formValues.email} onChange={handleChange} placeholder="Email" required className="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:ring-2 focus:ring-purple-500 outline-none transition" />
                  <input name="phone" value={formValues.phone} onChange={handleChange} placeholder="Phone Number" required className="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:ring-2 focus:ring-purple-500 outline-none transition" />
                </div>
                <input name="city" value={formValues.city} onChange={handleChange} placeholder="Target City" required className="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:ring-2 focus:ring-purple-500 outline-none transition" />
                <textarea name="message" value={formValues.message} onChange={handleChange} placeholder="Briefly describe your business interest..." rows="3" className="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-200 focus:ring-2 focus:ring-purple-500 outline-none transition" />
                
                <button type="submit" disabled={isSubmitting} className="w-full py-4 rounded-full bg-orange-500 text-white font-bold text-lg hover:bg-orange-600 transition shadow-xl disabled:opacity-50">
            ...
            <div>
              ...
              <div className="relative space-y-8">
                ...
                {processSteps.map((step, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative flex items-start gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-black text-sm mt-1 font-semibold leading-relaxed">{step.desc}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
      ...
      <section className="py-28 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <h2 className="text-4xl md:text-5xl font-extrabold text-purple-800 text-center mb-16">Franchise Investment & ROI</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {roi.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} whileHover={{ y: -8, scale: 1.04 }} className="relative bg-gradient-to-r from-purple-200 to-orange-200 backdrop-blur-xl border border-white/20 rounded-3xl p-10 shadow-2xl text-white overflow-hidden">
                ...
                <h4 className="text-lg font-semibold text-purple-800">{item.title}</h4>
                <p className="text-4xl font-extrabold text-black mt-3">{item.value}</p>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-5xl mx-auto px-6">
          <div className="bg-gradient-to-r from-purple-800 to-orange-500 rounded-[2.5rem] p-16 text-center text-white shadow-2xl">
            <h2 className="text-4xl md:text-5xl font-extrabold mb-6">Start Your Preschool Journey With Us</h2>
            <p className="text-white/90 text-lg mb-10 max-w-2xl mx-auto">
              Partner with a trusted preschool brand and build a profitable, future-ready educational institution.
            </p>
            <button onClick={scrollToEnquiry} className="px-12 py-4 rounded-full bg-white text-purple-800 font-bold text-lg hover:scale-105 transition">
              Enquire Now
            </button>
          </div>
        </div>
